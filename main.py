...
bot = telebot.TeleBot("7464820249:AAHV3p945y5uiCo65rYGyctrmQuZAkaR-JE")


def generate_keyboard():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    btn1 = types.KeyboardButton("/register")
    btn2 = types.KeyboardButton("/add_task")
    btn3 = types.KeyboardButton("/all_tasks")
    # btn4 = types.KeyboardButton("/task")
...
def get_age(mess):
    ...
    con.execute(ins)
    con.commit()
    bot.send_message(user, f"Введите вашу почту:")
...
def start_add_task(mess):
    user = mess.from_user.id
    bot.send_message(user, "Введите названия задачи:")
...
def all_tasks(mess):
    ...
    bot.send_message(user, "Чтобы посмотреть подробную информацию о задаче, используйте команду /task <id_задачи>",)

def generate_keyboard_succes():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def get_task(mess):
    ...
    if not task_data:
        ...
        return
    task = task_data[-1]
    markup = generate_keyboard_succes()
    bot.send_message(user, f"ID: {task[0]}\nНазвание: {task[2]} \nОписание: {task[3]} \nСтатус: {task[6]}", reply_markup=markup)
...
def succes_task(mess, task):
    ...
    if mess.text == "Выполнено":
        ...
        con.execute(ins)
        con.commit()
        bot.send_message(user, "Задаче присвоен статус Выполнено", reply_markup=generate_keyboard())
    else:
        bot.send_message(user, "Хорошо, продолжим работу", reply_markup=generate_keyboard())
